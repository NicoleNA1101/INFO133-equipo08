...
try:
    with connection.cursor() as cursor:
        ...
        with open(csv_file3, 'r') as file:
            ...
            header = next(csv_data)
            insert_query = '''
            INSERT INTO Comuna (ID_C, Nombre_P, PoblacionT, Nombre_C, Superficie_C, Densidad_C, ID_R)
            VALUES (%s,%s,%s,%s,%s,%s,%s)
            '''
            for row in csv_data:
                #print((row[0],row[2],row[4],row[1]))
                try:
                    cursor.execute(insert_query, (row[0],row[2],row[5],row[1],row[4],row[6],row[3]))
